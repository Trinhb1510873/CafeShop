...
class MonAnExport implements FromView, WithDrawings, WithEvents, ShouldAutoSize
{
    use Exportable, RegistersEventListeners;
    
    public function view(): View {
        return view('backend.monan.excel', [
            'danhsachsanpham' => MonAn::all(),
            'danhsachntd' => NhomThucDon::all(),
            'danhsachdvt' => DonViTinh::all()
        ]);
    }
    public function drawings() {
        ...
        $drawingLogo->setPath(public_path('storage/photos/hhd.jpg'));
        $drawingLogo->setHeight(90);
        $drawingLogo->setOffsetY(10);
        $drawingLogo->setCoordinates('D4');
        $offsetX = 40;
        $drawingLogo->setOffsetX($offsetX);
        $arrDrawings[] = $drawingLogo;
        $startRow = 7;
        $ds_sanpham = MonAn::all();
        foreach($ds_sanpham as $index=>$sp)
        {
            $drawing = new \PhpOffice\PhpSpreadsheet\Worksheet\Drawing();
            $drawing->setName($sp->ma_ten);
            $drawing->setDescription($sp->ma_dienGiai);
            $drawing->setPath(public_path('storage/photos/' . $sp->ma_hinh));
    ...
    */
    public static function afterSheet(AfterSheet $event){
        ...
        $event->sheet->getDelegate()->getStyle('A5:G6')->applyFromArray(
            [
                'font' => [
                    'bold' => true,
                ],
